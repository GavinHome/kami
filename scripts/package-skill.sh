...
awk '
  ...
  /^scripts\/(add_outline|build|check-update|checks|content|ensure-fonts|ensure_mathjax|highlight|lint|math_render|mathjax_svg|mcp_server|mermaid_normalize|optional_deps|render|shared|site_facts|tokens|verify|visual)\.(py|sh|js)$/ { print; next }
